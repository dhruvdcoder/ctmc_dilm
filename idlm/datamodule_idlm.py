...
from .types_idlm import IdlmBatch
...
NoisingType = Literal["joint", "independent"]


def _sample_n_drops_from_noise_schedule(
    noise_schedule: NoiseSchedule, t: float, seq_len: int
) -> int:
    """Sample number of drops from noise schedule given time t (joint / Poisson)."""
...
def _sample_n_drops_independent(
    noise_schedule: NoiseSchedule, t: float, seq_len: int
) -> int:
    """Independent noising: retained count m ~ Binomial(n, rho), rho = exp(-bar_sigma_0,t)."""
    if seq_len <= 0:
        return 0
    t_tensor = torch.tensor([t])
    _noise_rate, total_noise = noise_schedule(t_tensor)
    rho = float(torch.exp(-total_noise).item())
    rho = min(max(rho, 1e-12), 1.0 - 1e-12)
    m = int(np.random.binomial(seq_len, rho))
    n_drops = seq_len - m
    return min(max(0, n_drops), seq_len)
...
def idlm_drop_fn(
    ...
    elif noising == "independent":
        _n_drops = _sample_n_drops_independent(
            noise_schedule, t, _orig_seq_len
        )
    else:
        raise ValueError(f"Unknown noising type: {noising}")
...
def idlm_single_segment_collate_target_fn(
    ...
    # Create sparse tensor for targets (coalesce so duplicate (b,s,v) sum — same gap, same token)
...
class DefaultIdlmCollator(Collator):
    ...
    def __init__(
        ...
        Args:
            ...
            noising: ``joint`` (Poisson drops) or ``independent`` (Binomial retained length).
        """
...
class IdlmSeq2SeqCollator:
    ...
    def __init__(
        ...
        Args:
            ...
            truncate: Truncation strategy.
            noising: ``joint`` or ``independent`` (Binomial) noising for the suffix.
        """
        self.tokenizer = tokenizer
        self.block_size = block_size
        self.noise_schedule = noise_schedule
...
def print_batch_idlm(
    ...
    if batch.get("cls_position", None) is not None:
        print("cls_position:")
        print(batch["cls_position"][0])

    if batch.get("t", None) is not None:
        print("t:")
        print(batch["t"][0])
    if batch.get("noise_rate", None) is not None:
        print("noise_rate:")
        print(batch["noise_rate"][0])
    if batch.get("total_noise", None) is not None:
        print("total_noise:")
