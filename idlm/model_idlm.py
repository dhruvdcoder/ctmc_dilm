...
class LayerNormAndScale(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        # disable autocast for layer norm for numerical stability
...
class DDitFinalLayer(nn.Module):
    def __init__(
        ...
        with torch.no_grad():
            self.linear.weight.normal_(0.0, 0.001)
...
class DDitFinalLayerForClassification(nn.Module):
    ...
    def forward(self, x: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        ...
        shift_mlp, scale_mlp = self.adaLN_modulation(c)
        gate_mlp = None
        # AdaLN -> MLP -> dropout -> scale -> residual
        x = add_bias_apply_dropout_scale(
            self.mlp(
...
class DDITIDLMModel(torch.nn.Module, Model):
    def __init__(
        ...
        self.output_layer = DDitFinalLayer(
            d_model, num_embeddings, self.d_cond
        )
    ...
    def _inner(self, x, c, attention_mask, positions, cls_position):
    ...
    def forward(
        ...
        with torch.autocast(
                ...
                vocab_logits, length_logits = self._inner(
                    x, c, attention_mask, positions, cls_position
                )
    ...
    #    others = []
    #    for name, param in self.named_parameters():
    #        if "adaLN_modulation" in name or "ada_ln_modulations" in name:
    #            logger.info(f"{name}: decay 0.1")
    #            adaln_params.append(param)
    #        elif "bias" in name or "norm" in name:
    #            logger.info(f"{name}: decay 0.0")
    #            biases.append(param)
    #        else:
    #            logger.info(f"{name}: decay default")
    #            others.append(param)
    #    return [
    #        {"params": others},
    #        {
